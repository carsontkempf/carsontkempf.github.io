...
class AppleMusicService {
    ...
    /**
     * Initialize service with Worker configuration
     */
    async initialize() {
        try {
            // Wait for Worker config to be ready
            if (!window.learnWorkerConfig) {
                await new Promise((resolve, reject) => {
                    const timeout = setTimeout(() => reject(new Error('Worker config timeout')), 5000);
            ...
            const response = await fetch(window.learnWorkerConfig.getEndpoint('/config'));
            if (!response.ok) {
                throw new Error('Worker not accessible');
            }

            console.log('✅ Apple Music Service initialized with Worker');
            return true;
        } catch (error) {
    ...
    /**
     * Request JWT token from Worker (private key stays on server)
     */
    async requestTokenFromWorker() {
        try {
            if (!window.learnWorkerConfig) {
                throw new Error('Worker configuration not loaded. Include worker-config.js before this script.');
            }

            const response = await fetch(window.learnWorkerConfig.getEndpoint('/auth/apple-jwt'), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
            ...
            if (!data.success || !data.token) {
                throw new Error(data.error || 'No token returned from Worker');
            }

            console.log('🔑 Apple Music JWT generated by Worker');
            return data.token;
        } catch (error) {
            throw new Error(`Failed to get Apple Music token from Worker: ${error.message}`);
        }
    }
    ...
    async getDeveloperToken() {
        ...
        // Request new token from Worker (private key stays on server)
        try {
            this.token = await this.requestTokenFromWorker();
            this.tokenExpiry = Date.now() + (23 * 60 * 60 * 1000); // 23 hours

            console.log('🎵 New Apple Music developer token generated by Worker');
            return this.token;
        } catch (error) {
            throw new Error(`Failed to get Apple Music token: ${error.message}`);
        }
    }
...
// Auto-initialize when DOM is loaded (Worker config should be available)
